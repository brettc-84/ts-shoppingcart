...
export class BasketController extends BaseHttpController {
    ...
    private async addBasketItem(
        ...
        try {
            ...
            res.json(this._basketService.addBasketItem(basketId, itemToAdd)).status(
                201
            );
        } catch (err) {
            res.status(500).json({
    ...
    private async removeItem(
        @requestParam("basketId") basketId: string,
    ...
    private async updateBasketItem(
        @requestParam("basketId") basketId: string,
        @requestParam("itemId") itemId: string,
        @request() req: express.Request,
        @response() res: express.Response
